/* =========================================================
   MADIRA ADMIN.JS — FINAL
   ✅ Admin reads sales directly from Firebase:
      shops/madira/orders
   ✅ Works on hosted site / different devices
   ✅ No dependency on Cashier and Admin being open together
   ✅ Supports Stock Purchases + linked barcodes
   ========================================================= */
...
let FB = {
  ...
  auth: null,
  uid: null,
  unsub: { cats:null, prods:null, orders:null, purchases:null }
};
...
function _shopRef(){
  return FB.db.collection("shops").doc(FIREBASE_SHOP_ID);
}

function _catsRef(){
  return _shopRef().collection("categories");
}

function _prodsRef(){
  return _shopRef().collection("products");
}

function _ordersRef(){
  return FB.db.collection("shops").doc("madira").collection("orders");
}

function _purchasesRef(){
  return _shopRef().collection("purchases");
}
...
async function fbInitAndSync(){
  try {
    ...
    FB.enabled = false;
    alert("Firebase connection failed. Admin will show only local cached data.");
    return false;
  }
}
...
function fbStartRealtimeSync(){
  ...
  }, err => console.error("Admin categories sync error:", err));
  ...
  /*
    ✅ FINAL MAIN FIX:
    Admin listens directly to Firebase:
    shops/madira/orders
    So it works on hosted site and across devices.
  */
  FB.unsub.orders = _ordersRef()
    ...
    .onSnapshot((snap)=>{
      ...
      localStorage.setItem(LS_ORDERS, JSON.stringify(sortOrdersDesc(orders)));

      refreshOrderScreens();

      console.log("✅ Admin orders synced from Firebase:", orders.length);
    }, (err)=>{
      console.error("❌ Admin orders listener failed:", err);
      alert("Admin cannot read Firebase orders. Check Firestore rules and path: shops/madira/orders");
    });
  ...
  FB.unsub.purchases = _purchasesRef()
    ...
    }, err => console.error("Admin purchases sync error:", err));
}
...
function setCategories(cats){
  ...
  if (FB.enabled){
    fbSyncCategories(next).catch(e => {
      console.error("Category sync failed:", e);
      alert("Firebase category save failed.");
    });
  }
}

async function fbSyncCategories(nextCats){
  if (!FB.enabled) return;

  const existingSnap = await _catsRef().get();
  const batch = FB.db.batch();
...
function setProducts(prods){
  ...
  if (FB.enabled){
    fbSyncProducts(next).catch(e => {
      console.error("Product sync failed:", e);
      alert("Firebase product save/delete failed.");
    });
  }
}

async function fbSyncProducts(nextProds){
  if (!FB.enabled) return;

  const existingSnap = await _prodsRef().get();
  const batch = FB.db.batch();
...
function normalizeOrder(o){
  ...
  const tsISO =
    x.ts?.toDate ? x.ts.toDate().toISOString() :
    x.tsISO || x.ts || "";
  ...
  return {
    ...
    ts: tsISO,
    tsISO,
    receiptId: x.receiptId || x.id || x.__docId || "—",
    amount: Number(x.amount || 0),
    subtotal: Number(x.subtotal || 0),
    tax: Number(x.tax || 0),
    items: Array.isArray(x.items) ? x.items : [],
    status: x.status || "COMPLETED",
    method: x.method || "CARD",
    cashier: x.cashier || "Cashier",
...
function sortOrdersDesc(list){
  return [...(list || [])].sort((a,b)=>{
    const da = new Date(a.ts || a.tsISO || 0).getTime();
    const db = new Date(b.ts || b.tsISO || 0).getTime();
    return db - da;
  });
}
...
function bindOrderCacheUpdates(){
  ...
  if ("BroadcastChannel" in window){
    ...
    channel.onmessage = (event) => {
      ...
      if (!order.receiptId || order.receiptId === "—") return;

      const current = getOrders();
      const next = current.filter(o => o.receiptId !== order.receiptId);
      next.unshift(order);
...
function forceAllTableHeaders(){
  forceGrid(document.querySelector("#route-dashboard .table .thead"), "0.8fr 0.8fr 1.6fr 0.6fr 0.6fr 0.4fr");
  forceGrid(document.querySelector("#route-inventory .table .thead"), "2.2fr 1fr 0.7fr 0.6fr 0.8fr 0.7fr");
  forceGrid(document.querySelector("#route-orders .table .thead"), "1fr 1.1fr 1.7fr 0.8fr 0.8fr 0.8fr 0.7fr");
  forceGrid(document.querySelector("#route-reports .table .thead"), "0.6fr 1.6fr 1fr 0.7fr 0.9fr");
...
function renderDashboard(){
  ...
  const todays = orders.filter(o => (o.ts || o.tsISO || "").slice(0,10) === todayKey);
  ...
  if (dashBody){
    ...
    orders.slice(0,6).forEach(o => {
      ...
      row.innerHTML = `
        ...
        <div>${escapeHtml(o.timeLabel || fmtTime(o.ts || o.tsISO))}</div>
...
function drawTrendChart(){
  ...
  for (let i = days - 1; i >= 0; i--){
    ...
    const revenue = orders
      .filter(o => (o.ts || o.tsISO || "").slice(0,10) === key)
    ...
    row.innerHTML = `
      <div style="display:flex; gap:10px; align-items:center;">
        ...
        <div>
          ...
          ${
            Array.isArray(p.barcodes) && p.barcodes.length
            ? `<div style="font-size:12px;color:rgba(17,24,39,.55)">Other Barcodes: ${escapeHtml(p.barcodes.slice(0,3).join(", "))}${p.barcodes.length > 3 ? "..." : ""}</div>`
            : ``
          }
        </div>
      </div>
...
function saveNewProduct(){
  ...
  const exists = prods.some(p => {
    const primary = String(p.barcode || "").trim();
    const aliases = Array.isArray(p.barcodes) ? p.barcodes.map(x => String(x || "").trim()) : [];

    return primary === barcode || aliases.includes(barcode);
  });

  if (exists){
    alert("Barcode already exists. If it is the SAME item, use Stock Purchases and link barcode instead.");
    return;
  }
...
function bindOrdersControls(){
  ...
  document.getElementById("ordPreset")?.addEventListener("change", syncOrdersDateUI);
...
function renderOrders(){
  ...
  const todayKey = isoDateKey(new Date());
  const todays = ordersAll.filter(o => (o.ts || o.tsISO || "").slice(0,10) === todayKey);

  const todayRevenue = todays.reduce((s,o)=> s + num(o.amount), 0);

  setText("ordTodayCount", String(todays.length));
  setText("ordTodayRevenue", money(todayRevenue));
  setText("ordRefunds", String(todays.filter(o => String(o.status || "") === "REFUNDED").length));
  ...
  if (state.ordPreset !== "custom"){
    ...
    list = filterByDate(ordersAll, r.from, r.to);
  } else {
    list = filterByDate(
      ordersAll,
  ...
  list.forEach(o => {
    ...
    row.innerHTML = `
      ...
      <div>
        ...
        <div style="font-size:12px;color:rgba(17,24,39,.6)">${escapeHtml(o.timeLabel || fmtTime(o.ts || o.tsISO))}</div>
      </div>
...
function bindReportsControls(){
  ...
  document.getElementById("repApply")?.addEventListener("click", ()=>{
    state.repPreset = document.getElementById("repPreset")?.value || "all";
...
function renderReports(){
  ...
  if (state.repPreset !== "custom"){
    ...
    orders = filterByDate(getOrders(), r.from, r.to);
  } else {
    orders = filterByDate(
      getOrders(),
    ...
    <div class="form" style="padding:0;">
      ...
      <div class="fieldRow">
        ...
        <div class="field">
          ...
          <input id="mGst" type="number" min="0" max="100" step="0.01" value="${escapeAttr(isFinite(Number(p.gstRate)) ? Number(p.gstRate) : "")}" />
        </div>
        <div class="field">
          <label>Stock</label>
      ...
      <div class="note" style="margin-top:10px;">
        Other linked barcodes are managed automatically from Stock Purchases.
      </div>
    </div>
  `;
      ...
      <div class="modalBody">
        <div class="form" style="padding:0;">
          ...
          <div class="field" style="margin-top:10px;">
            ...
            <div class="note" style="margin-top:8px;">Keep OFF normally.</div>
          </div>
        </div>
      </div>
...
async function savePurchaseEntry(){
  ...
  try {
    ...
    alert("Purchase save failed. Check console.");
    return;
  }
  ...
  orders.forEach(o => {
    (o.items || []).forEach(it => {
      flat.push({
        ...
        orderSubtotal: round2(num(o.subtotal)),
        orderTax: round2(num(o.tax)),
...
function hydrateAdminIdentity(){
  ...
  try {
    if (typeof window.getSession === "function") sess = window.getSession();
  } catch {}
...
function filterByDate(list, from, to){
  ...
  return list.filter(o => {
    const ts = o.ts ? new Date(o.ts) : (o.tsISO ? new Date(o.tsISO) : null);
